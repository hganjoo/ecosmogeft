...
!     coarse sf             N/A           active_mg(myid,ilevel)%u(:,5)
!     coarse density        N/A           active_mg(myid,ilevel)%u(:,6)
! ------------------------------------------------------------------------
...
subroutine multigrid_fine(ilevel,icount)
   ...
   integer  :: ind,igrid_mg,igrid_amr,icell_amr
...
!  if(nboundary>0) levelmin_mg=max(levelmin_mg,2)
   ...
   !levelmin_mg = levelmin  ! switch multigrid-solver off, TODO
   ...
   main_iteration_loop: do
      ...
      ! Verbosity
      if(verbose) print '(A,I5,A,1pE10.3)','   ==> Step=',iter,' Error=',err
...
!  if(ilevel.eq.levelmin .and. aexp>0.33) stop

   ! ---------------------------------------------------------------------
   ! Cleanup or restore MG levels after solve complete
   !
   ! If extradof is to be solved:
   !                                                                      
   ! Do NOT cleanup MG levels after Poisson solver completes, because this
   ! will be used for the extradof solver later! Instead simply reset some
   ! container of the multigrid solver to 0 for the use of extradof solver
   ! ---------------------------------------------------------------------
   if(extradof) then
      do ifine=1,ilevel-1
...
subroutine build_parent_comms_mg(active_f_comm,ifinelevel)
   ...
   if(nact_tot>0) then
      ...
      ! u needs to be one dimension bigger as extradof is to be solved
      if(extradof) then
