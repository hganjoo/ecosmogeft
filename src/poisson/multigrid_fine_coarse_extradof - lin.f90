         ...
         do igrid_mg=1,ngrid,nvector
            ...
            do i=1,nbatch
               ...
               if(.not. btest(active_mg(myid,ilevel)%f(icell_mg(i),1),0)) then
                  ...
                  ! Chr 30/03/20
                  eta = eta + (qv/rv)*1.5d0*Ia*omega_m*aexp*active_mg(myid,ilevel)%u(icell_mg(i),6)
                  ...
                  if((pv/rv)>=0.d0) then
                     ...
                     op = op - 1.5d0*omega_m*(xiv/nuv)*aexp*active_mg(myid,ilevel)%u(icell_mg(i),6)*dx2 - active_mg(myid,ilevel)%u(icell_mg(i),2)*dx2
                  else
                     !op  = op - (0.75d0*(-(pv/rv)-dsqrt(eta)) - sf_src_mean)*dx2 - &
                     !    & active_mg(myid,ilevel)%u(icell_mg(i),2)*dx2
                     op = op - 1.5d0*omega_m*(xiv/nuv)*aexp*active_mg(myid,ilevel)%u(icell_mg(i),6)*dx2 - active_mg(myid,ilevel)%u(icell_mg(i),2)*dx2
         ...
         do igrid_mg=1,ngrid,nvector
            ...
            do i=1,nbatch
               ...
               if(.not. btest(active_mg(myid,ilevel)%f(icell_mg(i),1),0)) then
                  ...
                  if((pv/rv)>=0.d0) then
                     !op  = op - (0.75d0*(-(pv/rv)+dsqrt(eta)) - sf_src_mean)*dx2 
                     op = op - 1.5d0*omega_m*(xiv/nuv)*aexp*active_mg(myid,ilevel)%u(icell_mg(i),6)*dx2
                  else
                     !op  = op - (0.75d0*(-(pv/rv)-dsqrt(eta)) - sf_src_mean)*dx2
                     op = op - 1.5d0*omega_m*(xiv/nuv)*aexp*active_mg(myid,ilevel)%u(icell_mg(i),6)*dx2
                  end if
