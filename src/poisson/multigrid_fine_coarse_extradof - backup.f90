         ...
         do igrid_mg=1,ngrid,nvector
            ...
            do i=1,nbatch
               ...
               if(.not. btest(active_mg(myid,ilevel)%f(icell_mg(i),1),0)) then
                  ...
                  ! Chr 30/03/20
                  eta = rv*eta + (qv)*1.5d0*Ia*omega_m*aexp*active_mg(myid,ilevel)%u(icell_mg(i),6)
                  ...
                  if((pv/rv)>=0.d0) then
                     ...
                     op  = op - (0.75d0*(-(pv/rv)-dsqrt(eta))/rv - sf_src_mean)*dx2 - &
                         & active_mg(myid,ilevel)%u(icell_mg(i),2)*dx2
                  end if
         ...
         do igrid_mg=1,ngrid,nvector
            ...
            do i=1,nbatch
               ...
               if(.not. btest(active_mg(myid,ilevel)%f(icell_mg(i),1),0)) then
                  ...
                  !eta = alpha_sf**2 + eta*8.0d0/3.0d0
                  ! Chr 30/03/20
                  eta = eta*rv + (qv)*1.5d0*Ia*omega_m*aexp*active_mg(myid,ilevel)%u(icell_mg(i),6)
                  eta = (pv)**2 + 8.0d0/3.0d0*rv*eta
