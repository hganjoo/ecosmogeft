         ...
         do igrid_mg=1,ngrid,nvector
            ...
            do i=1,nbatch
               ...
               if(flag2(icell_amr(i))/ngridmax==0) then
                  ...
                  ! Chr 30/03/20
                  eta = eta + (qv/rv)*1.5d0*Ia*omega_m*aexp*(rho(icell_amr(i))-rho_tot)
                  eta = (pv/rv)**2 + 8.0D0/3.0D0*eta
                  ...
                  if((pv/rv)>=0.0d0) then
                     ! derived form Eq.(69)
                     !op = op-(0.75d0*(-(pv/rv)+dsqrt(eta))-sf_src_mean)*dx2
                     op = op - 1.5d0*omega_m*(xiv/nuv)*aexp*(rho(icell_amr(i))-rho_tot)*dx2
                  else
                     ! derived form Eq.(69)
                     !op = op-(0.75d0*(-(pv/rv)-dsqrt(eta))-sf_src_mean)*dx2
                     op = op - 1.5d0*omega_m*(xiv/nuv)*aexp*(rho(icell_amr(i))-rho_tot)*dx2
         ...
         do igrid_mg=1,ngrid,nvector
            ...
            do i=1,nbatch
               ...
               if(flag2(icell_amr(i))/ngridmax==0) then
                  ...
                  op = op-(param_b3*(bf_src1+bf_src2+bf_src3+bf_src4)/dx**5-sf_src_mean)*dx2
...
subroutine gauss_seidel_mg_fine_extradof(ilevel,isf,redstep)
   ...
   integer  :: iskip_amr
   ! verbose test
   integer  :: tti,ttind,ttiskip,ttngrid,ttigrid_amr,tticell_amr
   integer  :: ix,iy,iz,countc1,countc2
   real(dp) :: xs,ys,zs
         ...
         do igrid_mg=1,ngrid,nvector
            ...
            do i=1,nbatch
               ...
               if(flag2(icell_amr(i))/ngridmax==0) then
                  ...
                  if((pv/rv)>=0.0d0) then
                     ! derived form Eq.(69)
                     !sf_src(icell_amr(i)) = 0.75d0*(-(pv/rv)+dsqrt(eta))
                     !op = op - (0.75d0*(-(pv/rv)+dsqrt(eta))-sf_src_mean)*dx2
                     !sf(icell_amr(i)) = sf(icell_amr(i))-op/dop
                     ! linear theory for debugging
                     op = op - 1.5d0*omega_m*(xiv/nuv)*aexp*(rho(icell_amr(i))-rho_tot)*dx2
                     sf_src(icell_amr(i)) = 1.5d0*omega_m*(xiv/nuv)*aexp*(rho(icell_amr(i))-rho_tot)*dx2
                  else
                     ! derived form Eq.(69)
                     !sf_src(icell_amr(i)) = 0.75d0*(-(pv/rv)-dsqrt(eta))
                     !op = op - (0.75d0*(-(pv/rv)-dsqrt(eta))-sf_src_mean)*dx2
...
subroutine restrict_density_fine_reverse_extradof(ifinelevel,isf)
...
!        ind_c_cell=(icell_c_amr-ncoarse-1)/ngridmax+1
         ...
         if(isf.eq.1) then
            rho1=(rho(icell_f_amr)-rho_tot)/(dtwotondim-dble(n_masked(igrid_f_mg)))
         else
...
subroutine interpolate_and_correct_fine_extradof(ifinelevel,isf)
   use amr_commons
   use poisson_commons
   use extradof_commons
   use extradof_parameters
...
!              ind_c       = (icell_c_amr-ncoarse-1)/ngridmax + 1
         ...
         if(isf.eq.1) then
            ...
            do i=1,nbatch
               cbf(icell_amr(i),isf-1) = cbf(icell_amr(i),isf-1) + corr(i)
            end do
         endif
      end do
   end do
