...
subroutine multigrid_fine_extradof(ilevel,icount,isf)
   ...
   if(verbose) print '(A,I2)','V-cycle extradof: entering fine multigrid at level ',ilevel
...
!   endif
   ...
   main_iteration_loop: do
      ...
      do i=1,ngs_fine_extradof_pre
         ...
         !if (verbose) write(*,*) "SF SRC made. Entering GS..."
         call gauss_seidel_sf_src_mean_extradof(ilevel,isf)
         !if (verbose) write(*,*) "SF SRC solved. Entering GS Black..."
         ...
         call make_virtual_fine_dp(sf_src(1),ilevel)
         !if (verbose) write(*,*) "Calling GS SRC..."
         call gauss_seidel_sf_src_mean_extradof(ilevel,isf)
      end do
      ...
      ! Verbosity
      if(verbose) print '(A,I4,A,1e13.6,A,1e13.6)',' extradof   ==> Step=',iter,', Residual =',residual,', Truncation error= ',trunc_err
      !print '(A,I4,A,1e13.6,A,1e13.6)','   ==> Step=',iter,', Residual =',residual,', Truncation error= ',trunc_err
...
!     end if
   ...
   if(myid==1) print '(A,I5,A,I5,A,1E15.6, A, 1e15.6)',' extradof   ==> Level=',ilevel,' Step=',&
...
recursive subroutine recursive_multigrid_coarse_extradof(ifinelevel,isf,safe)
   ...
   if(verbose) then
      if(ifinelevel>levelmin_mg) then
         print '(A,I2)','V-cycle extradof: entering coarse multigrid at level ',ifinelevel
      else
         print '(A,I2)','V-cycle extradof: entering coarsest multigrid level ',ifinelevel
      end if
   end if
   ...
   if(ifinelevel<=levelmin_mg) then
      ...
      if(verbose) print '(A,I2)','V-cycle extradof: leaving coarsest multigrid level ',ifinelevel
      return
   end if
   ...
   if(verbose) print '(A,I2)','V-cycle extradof: leaving coarse multigrid at level ',ifinelevel
...
subroutine gauss_seidel_sf_src_mean_extradof(ilevel,isf)
   ...
   !if(verbose) write(*,*) 'test_src is',test_src
   ...
   !if(verbose) write(*,*) 'The average source is',test_srcbar,isf,test_src
   ! TODO
